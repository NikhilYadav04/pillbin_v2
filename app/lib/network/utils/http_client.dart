...
class HttpClient {
  ...
  static const String _vendorCenterIdKey = 'vendor_center_id';
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        ...
        onError: (error, handler) async {
          ...
          //* Handle 401 Unauthorized (skip retry for refresh calls themselves)
    ...
    try {
      ...
      final response = await _dio.post(
        ...
        options: Options(
          ...
          // Skip the interceptor retry to avoid infinite loop on 401
  ...
  Future<String> getRole() async {
    return await _secureStorage.read(key: _roleKey) ?? 'user';
  }

  Future<void> saveVendorCenterId(String? centerId) async {
    if (centerId != null && centerId.isNotEmpty) {
      await _secureStorage.write(key: _vendorCenterIdKey, value: centerId);
    } else {
      await _secureStorage.delete(key: _vendorCenterIdKey);
    }
  }

  Future<String?> getVendorCenterId() async {
    return await _secureStorage.read(key: _vendorCenterIdKey);
  }
}
