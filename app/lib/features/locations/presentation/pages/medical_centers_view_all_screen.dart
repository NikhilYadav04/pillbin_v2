...
class MedicalCentersViewAllScreen extends StatefulWidget {
  const MedicalCentersViewAllScreen({super.key});

  @override
  State<MedicalCentersViewAllScreen> createState() =>
      _MedicalCentersViewAllScreenState();
}

class _MedicalCentersViewAllScreenState
    extends State<MedicalCentersViewAllScreen> {
  ...
  void _applyFiltersSearch() {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      ...
      if (_filtered.isEmpty) {
        provider.resetAllCenters();
        provider.searchMedicalCenters(
            query: searchQuery, facilityType: "");
      } else {
        provider.updateFilterSearch(_filtered);
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<MedicalCenterProvider>(
            ...
            children: [
              ...
              Expanded(
                child: provider.isLoading &&
                        (provider.allCenters.isEmpty || provider.isSearchAPI)
  ...
  Widget _buildResultsCount(
    ...
    return Container(
      ...
      child: Row(
        children: [
          Text(
            '${provider.allCenters.length} centers found',
            style: PillBinMedium.style(
              fontSize: isTablet ? sw * 0.02 : sw * 0.032,
              color: PillBinColors.textSecondary,
            ),
  ...
  Widget _buildCentersList(
      double sw, double sh, MedicalCenterProvider provider) {
    if (provider.allCenters.isEmpty) {
      return Center(
        child: Column(
    ...
    //* get user's saved medical centers list
    List<String> userSavedMedicalCenters =
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: provider.allCenters.length + (provider.hasMore ? 1 : 0),
        itemBuilder: (context, index) {
          //* 👇 Handle pagination loader at the end
          if (index == provider.allCenters.length) {
            return Padding(
          ...
          final center = provider.allCenters[index];

          return LocationCard(
            sh: sh,
            sw: sw,
            medicalCenter: center,
            isSaved: userSavedMedicalCenters.contains(center.id),
          );
        },
      ),
    );
