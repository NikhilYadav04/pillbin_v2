...
class _SavedMedicalCentersScreenState extends State<SavedMedicalCentersScreen> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels >=
        ...
        final provider = context.read<SavedCentersProvider>();
        if (provider.hasMore && !provider.isLoading) {
          provider.getSavedMedicalCenters();
        }
      }
    });

    // Reset filters on fresh entry
    _maxDistance = 10.0;
    _minRating = 0.0;
                  ...
                  Expanded(
                    child: OutlinedButton(
                      onPressed: () {
                        ...
                        //* Reset filters
                        context.read<SavedCentersProvider>().clearFilterSearch();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<SavedCentersProvider>(
            ...
            children: [
              ...
              Expanded(
                child: (provider.isLoading &&
                        (provider.centers!.isEmpty || provider.isDelete))
  ...
  Widget _buildResultsCount(
      double sw, double sh, bool isTablet, SavedCentersProvider provider) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            '${provider.savedTotal} saved centers',
            style: PillBinMedium.style(
              fontSize: isTablet ? sw * 0.02 : sw * 0.032,
              color: PillBinColors.textSecondary,
            ),
  ...
  Widget _buildCentersList(double sw, double sh, SavedCentersProvider provider) {
    if (provider.centers!.isEmpty) {
      return Center(
        child: Column(
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: provider.centers!.length + (provider.hasMore ? 1 : 0),
        itemBuilder: (context, index) {
          //* 👇 Handle pagination loader at the end
          if (index == provider.centers!.length) {
            return Padding(
          ...
          return LocationCard(
            ...
            isSaved: savedMedicalCenters.contains(center.id),
          );
        },
      ),
    );
