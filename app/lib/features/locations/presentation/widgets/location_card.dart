    ...
    return Row(
      children: [
        ClipRRect(
          ...
          child: widget.medicalCenter.imageUrls.isNotEmpty
              ? CachedNetworkImage(
                  imageUrl: widget.medicalCenter.imageUrls.first,
                  width: isTablet ? widget.sw * 0.1 : widget.sw * 0.16,
                  height: isTablet ? widget.sw * 0.1 : widget.sw * 0.16,
                  fit: BoxFit.cover,
                  placeholder: (_, __) => Container(
                    width: isTablet ? widget.sw * 0.1 : widget.sw * 0.16,
                    height: isTablet ? widget.sw * 0.1 : widget.sw * 0.16,
                    color: PillBinColors.greyLight,
                    child: Icon(_getFacilityIcon(),
              ...
              : Container(
                  padding: EdgeInsets.all(
                      isTablet ? widget.sw * 0.015 : widget.sw * 0.025),
                  decoration: BoxDecoration(
                    ...
                    onTap: () async {
                      if (!isSaved) {
                        ...
                        final result = await context
                            .read<SavedCentersProvider>()
                            .saveMedicalCenter(
                                medicalCenterId: widget.medicalCenter.id);
                        if (result == 'success' && mounted) {
                          userProvider.addSavedId(widget.medicalCenter.id);
                            ...
                            return AlertDialog(
                              ...
                              actions: [
                                ...
                                TextButton(
                                  onPressed: () async {
                                    final savedProvider =
                                        context.read<SavedCentersProvider>();
                                    final userProvider =
                                        context.read<UserProvider>();

                                    //* if already deletion in progress stop
                                    if (savedProvider.isDelete) {
                                      if (context.mounted) {
                                        CustomSnackBar.show(
                                            context: context,
                                            icon: Icons.warning,
                                            title:
                                                "Another Deletion in Progress!! Please Wait !!");
                                      }
                                    ...
                                    final response =
                                        await savedProvider.removeSavedMedicalCenter(
                                            medicalCenterId:
                                                widget.medicalCenter.id);

                                    if (response == 'success') {
                                      userProvider.removeSavedId(
                                          widget.medicalCenter.id);
                                    }
