  ...
  Future<void> _checkAuthAndNavigate() async {
    try {
      ...
      if (isAuth) {
        ...
        if (mounted) {
          final role = await _httpClient.getRole();
          final vendorCenterId = await _httpClient.getVendorCenterId();
          String destination;
          if (role == 'vendor') {
            destination = (vendorCenterId != null && vendorCenterId.isNotEmpty)
                ? '/vendor-bottom-bar-screen'
                : '/vendor-onboarding-screen';
          } else {
            destination = '/bottom-bar-screen';
          }
