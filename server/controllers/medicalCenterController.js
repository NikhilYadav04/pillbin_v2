...
//* Get the public inventory for a medical center
const getMedicalCenterInventory = async (req, res) => {
  try {
    const { id } = req.params;

    const medicalCenter = await MedicalCenter.findById(id).select(
      "name inventory acceptedMedicineTypes isVendorManaged isActive donationCount"
    );

    if (!medicalCenter || !medicalCenter.isActive) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Medical center not found" });
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        isVendorManaged: medicalCenter.isVendorManaged,
        donationCount: medicalCenter.donationCount,
        acceptedMedicineTypes: medicalCenter.acceptedMedicineTypes,
        inventory: medicalCenter.inventory,
      },
    });
  } catch (error) {
    console.error("Get medical center inventory error:", error);
    res.status(500).json({ statusCode: 500, message: "Server error" });
  }
};
